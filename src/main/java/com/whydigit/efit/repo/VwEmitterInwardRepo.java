...
public interface VwEmitterInwardRepo extends JpaRepository<VwEmitterInwardVO, Long>{
   ...
   @Query(nativeQuery = true,value= "select a.* from vw_emitter_inward a where a.org_id=?1 And a.emitter_id=?2 And a.flow_id=?3")
List<VwEmitterInwardVO> findAllByOrgIdFlow(Long orgId, Long emitterId, Long flowid);


   @Query(nativeQuery = true,value= "select a.* from vw_emitter_inward a where a.org_id=?1 And a.warehouse_location_id=?2")
