...
public interface TransportPickupRepo extends JpaRepository<TransportPickupVO, Long> {
	...
	@Query(nativeQuery = true, value = " SELECT docid,docdate,drivername,driverphoneno,transportdocno,vechicleno,fromstockbranch,tostockbranch,transporter,handoverby FROM transportpickup \r\n"
			...
			+ "		WHERE FIND_IN_SET(a.warehouseid, b.access_warehouse) > 0 and b.user_id=?2 and b.org_id=?1 group by whlocation)\r\n"
			+ "			group by docid,docdate,drivername,driverphoneno,transportdocno,vechicleno,fromstockbranch,tostockbranch,transporter,handoverby")
	...
	@Query(value = "select a.docid as TransactionNo ,a.docdate as TransactionDate,a.docdate as DispatchDate,b.displayname as sender,\r\n"
    		+ "concat(c.street1,' ,',c.street2,' ,',c.state) as senderAddress,c.gstin as SenderGst,d.name as Receiver,e.address As receiverAddress,e.gst as ReceiverGST\r\n"
