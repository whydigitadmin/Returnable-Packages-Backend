...
public interface OemBinInwardDetailsRepo extends JpaRepository<OemBinInwardDetailsVO, Long> {
	...
	@Query(value = "SELECT c.flowid,c.flow\r\n" + "FROM binoutward a\r\n"
			+ "JOIN flow c ON a.flowid = c.flowid\r\n"
			+ "JOIN users b ON FIND_IN_SET(a.flowid, b.access_flow_id) AND b.user_id = ?1 and b.org_id=?2 GROUP BY c.flowid,c.flow", nativeQuery = true)
